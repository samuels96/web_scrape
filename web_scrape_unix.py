...
def get_page(soup,pr,po):
    ...
    with open("{}/{}/{}.html".format(pr,po,po), "w" ) as handler:
            handler.write(str(soup))

    with open("{}/{}/{}_all.txt".format(pr,po,po), "w" , ) as handler:
            handler.write(str(soup))

def get_text(soup,pr,po):
    with open("{}/{}/{}_text.txt".format(pr,po,po), "w" ) as handler:
        for x in soup.find_all('p'):
            handler.write(x.get_text().encode('utf-8')+"\n")

    with open("{}/{}/{}_paragrarphs.html".format(pr,po,po), "w" ) as handler:
        for x in soup.find_all('p'):
            handler.write(str(x))
...
def get_img(soup,base_url,pr,po):
    ...
    with open("{}/{}/{}_img_links.txt".format(pr,po,po), "w" ) as handler:
        for x in img:
...
def get_all(soup,base_url,pr,po):
    p1 = Process(target=download_img,args=(soup,base_url,pr,po))
    p2 = Process(target=get_text,args=(soup,pr,po))
    p3 = Process(target=get_img,args=(soup,base_url,pr,po))
    p4 = Process(target=get_page,args=(soup,pr,po))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
...
def main():
    ...
    base_url = re.match(r"^(^https?://[^/]*)",url).groups()[0]
